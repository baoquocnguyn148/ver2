# Retail Sales Analytics Data Platform — AWS Data Lakehouse, BI Dashboard & Machine Learning

## 1. Tổng Quan Dự Án

Dự án này xây dựng một **data product end-to-end** cho chuỗi bán lẻ thiết bị điện tử: từ dữ liệu giao dịch thô trong `DB.xlsx`, pipeline Python chuẩn hóa thành **star schema**, xuất **curated Parquet** lên Amazon S3, đăng ký metadata qua **AWS Glue**, truy vấn bằng **Athena**, trực quan hóa bằng **Power BI**, và chạy **Machine Learning** để dự báo doanh thu/lợi nhuận cũng như ưu tiên khách hàng có rủi ro churn.

Project được thiết kế để thể hiện đồng thời ba nhóm năng lực:

| Năng lực | Nội dung thể hiện |
|---|---|
| **Data Engineering** | Local/S3 dual-mode IO, ETL, Data Quality Gate, Parquet partitioning, Glue Catalog, Athena views, GitHub Actions, Docker, Terraform guardrails |
| **Data Analytics** | Phân tích doanh thu, lợi nhuận, AOV, mùa vụ, sản phẩm, loyalty, anomaly 2020-Q1 và khuyến nghị hành động |
| **Machine Learning** | Forecast doanh thu/lợi nhuận, churn scoring, retention priority list, model artifacts và validation outputs |

| Chỉ số chính | Kết quả |
|---|---:|
| Fact transactions | 84,436 rows |
| Data model | 1 fact table + 5 dimension tables |
| Cloud query layer | Glue database + 11 Athena tables/views |
| Portfolio deploy | AWS Amplify public site |
| Automation | GitHub Actions manual/weekly pipeline |
...
| Area | Files / Folders | Vai trò |
|---|---|---|
| Core pipeline | `data_process.py`, `ml_pipeline.py` | ETL chính, tạo star schema, chạy forecast và churn scoring |
| ETL package | `src/etl/build_dimensions.py`, `src/etl/build_fact.py`, `src/etl/data_quality.py` | Tách logic tạo fact/dim và rule engine cho Data Quality |
| IO/config | `src/config.py`, `src/utils/io_utils.py` | Toggle `LOCAL_MODE`, đọc/ghi local hoặc S3 qua cùng một interface |
| Cloud scripts | `scripts/run_cloud_pipeline.py`, `scripts/register_glue_tables.py`, `scripts/repair_athena_tables.py`, `scripts/create_athena_views.py`, `scripts/validate_athena.py` | Orchestration, Glue registration, Athena repair/view/validation |
| SQL ELT | `sql/views/*.sql` | Analytics views tách khỏi Python để dễ review và reuse |
| CI/CD | `.github/workflows/pipeline.yml` | Unit tests, Docker build check, scheduled/manual cloud pipeline, artifact upload |
| IaC | `infra/terraform/` | S3, Glue, Athena, IAM policy, lifecycle/versioning guardrails |
| Optional extension | `airflow/dags/`, `dbt/` | Minh họa hướng mở rộng Airflow/dbt nhưng không bắt buộc deploy |
| Testing | `tests/` | Smoke tests, DQ tests, Glue empty-path guard tests |
...
```mermaid
graph TD
    A["BƯỚC 1<br>Extract & Transform<br>(data_process.py)"] --> B["BƯỚC 2<br>Data Quality Gate<br>(run_data_quality.py)"]
    B --> C["BƯỚC 3<br>ML Pipeline<br>(ml_pipeline.py)"]
    C --> D["BƯỚC 4<br>Register Glue Tables<br>(register_glue_tables.py)"]
    D --> E["BƯỚC 5<br>Repair Athena Partitions<br>(repair_athena_tables.py)"]
    E --> F["BƯỚC 6<br>Create Athena Views<br>(create_athena_views.py)"]
    F --> G["BƯỚC 7<br>Validate Athena<br>(validate_athena.py)"]
...
erDiagram
    ...
    DIM_Geography ||--o{ FACT_Sales : "located_in"
```
...
| Zone | S3 Prefix | Nội dung | Mục đích |
|---|---|---|---|
| Raw | `s3://ver2-retail-analytics/raw/DB.xlsx` | File nguồn Excel, bật S3 Versioning | Giữ dữ liệu gốc có thể rollback |
| Curated fact | `curated/fact_sales/partition_year=YYYY/` | Parquet fact table theo năm | Athena scan ít dữ liệu hơn khi lọc thời gian |
| Curated customer | `curated/dim_customer/partition_loyalty_status=.../` | Dimension khách hàng theo loyalty tier | Phân tích loyalty/retention |
| Curated product | `curated/dim_product/` | Dimension sản phẩm dạng Parquet | Join product/category/product line |
| Curated date | `curated/dim_date/partition_year=YYYY/` | Dimension thời gian | Quarter/year analytics |
| Curated geography | `curated/dim_geography/partition_country=.../` | Dimension địa lý | Market/country/province analytics |
| Outputs | `outputs/` | Forecast, churn, reports, models, images, DQ report | Artifacts để audit, portfolio và BI |
| Athena output | `outputs/athena/` | Query results | Lifecycle tự xóa sau 30 ngày để tiết kiệm chi phí |
...
| Step | Hành động | Lý do |
|---:|---|---|
| 1 | Checkout repository | Lấy source code mới nhất |
| 2 | Configure AWS credentials từ GitHub Secrets | Cho phép job ghi S3, Glue và Athena |
| 3 | Setup Python 3.12 với pip cache | Giảm thời gian cài dependencies |
| 4 | Install dependencies và tạo output folders | Chuẩn bị runtime |
| 5 | Run unit tests | Test gate, fail trước khi đụng tới AWS |
| 6 | Build Docker image | Validate Dockerfile/reproducibility mỗi lần chạy |
| 7 | Run `scripts/run_cloud_pipeline.py` | ETL, DQ, ML, Glue, Athena repair/views/validation |
| 8 | Upload reports as GitHub artifact | Giữ lại DQ/validation reports kể cả khi fail |
| 9 | Notify on failure | In link trực tiếp tới run log để debug nhanh |
...
| `ATHENA_WORKGROUP` | `primary` | Workgroup đang dùng cho demo thủ công; Terraform có thể tạo workgroup riêng `retail_analytics_workgroup` |
...
### Cloud Evidence: AWS Pipeline Run

Các ảnh dưới đây là bằng chứng end-to-end: dữ liệu đã lên S3, curated Parquet đã partition đúng, Glue/Athena đọc được dữ liệu, CI/CD chạy xanh, Amplify deploy public, và Budget guardrail đã bật.

<table>
  <tr>
    <td width="50%"><b>S3 Data Lake Root</b><br><img src="docs/images/cloud/s3-bucket-root.png" alt="S3 data lake root with raw, curated, outputs prefixes" width="420"></td>
    <td width="50%"><b>Fact Sales Partitions</b><br><img src="docs/images/cloud/s3-fact-sales-partitions.png" alt="S3 fact_sales parquet partitions by year" width="420"></td>
  </tr>
  <tr>
    <td><b>Pipeline Outputs & ML Artifacts</b><br><img src="docs/images/cloud/s3-outputs-artifacts.png" alt="S3 outputs with reports, DQ report and ML artifacts" width="420"></td>
    <td><b>Glue Data Catalog</b><br><img src="docs/images/cloud/glue-catalog-tables.png" alt="Glue catalog database with tables and Athena views" width="420"></td>
  </tr>
  <tr>
    <td><b>Athena Fact Row Count</b><br><img src="docs/images/cloud/athena-fact-count.png" alt="Athena fact_sales row count result" width="420"></td>
    <td><b>Athena Sales View</b><br><img src="docs/images/cloud/athena-sales-by-product.png" alt="Athena sales_by_product query result" width="420"></td>
  </tr>
  <tr>
    <td><b>Athena Retention Priority View</b><br><img src="docs/images/cloud/athena-retention-priority.png" alt="Athena retention priority query result" width="420"></td>
    <td><b>GitHub Actions CI/CD</b><br><img src="docs/images/cloud/github-actions-success.png" alt="Successful GitHub Actions workflow runs" width="420"></td>
  </tr>
  <tr>
    <td><b>AWS Amplify Public Deploy</b><br><img src="docs/images/cloud/amplify-deploy-success.png" alt="AWS Amplify deployed portfolio app" width="420"></td>
    <td><b>AWS Budget Guardrail</b><br><img src="docs/images/cloud/aws-budget-guardrail.png" alt="AWS budget guardrail healthy status" width="420"></td>
  </tr>
</table>
